<?php
if(!isset($_SESSION['admin_login']))
$miconexion = mysqli_connect("localhost", "id17552518_root", "GBou9x2FtB!!", "id17552518_mz_motorsports");
    if(!$miconexion) 
    {

        echo "La conexion ha fallado: " . mysqli_error();
        exit();
    }
# Se conecta a la base de datos donde se encuentran los posts ↑ ↑ ↑
# Comienza función de borrado ↓ ↓ ↓
if(isset($_GET['id']))
{
    $id = $_GET['id']; # Obtiene el ID de la publicación
    $res = mysqli_query($miconexion, "SELECT * FROM usuarios WHERE id = $id"); # Se selecciona la info respecto al ID de la publicación
    $query = "DELETE FROM usuarios WHERE id = $id";
    $result = mysqli_query($miconexion,$query);
    $query2 = "ALTER TABLE usuarios AUTO_INCREMENT = 1";
    $result2 = mysqli_query($miconexion,$query2);
    if(!$result)
    {
        die("El borrado ha fallado");
    }
    if(!$result2){
        die("El auto_increment ha fallado");
    }
}
# Termina función de borrado ↑ ↑ ↑
?>
<script type="text/javascript">
	alert("Se ha eliminado el usuario");
	window.location.href='usr_adm.php';
</script>